...
class PoolAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        x_ph = self.patch_pool1(x)  # (B, D, H, 4)

        # TODO: 2. Squeeze along w-axis; get X_pw ~ (B,D,4,W)
        x_pw = self.patch_pool2(x)  # (B, D, 4, W)

        # TODO: 3. Matrix multiplication; get X_1 = X_ph @ X_pw
        x_1 = torch.matmul(x_ph, x_pw)  # (B, D, H, W)
        ...
        x_pdh = self.embdim_pool1(x_0_)  # (B, N, 32, 4)

        # TODO: 2. Squeeze along Dw-axis; get X_PDw ~ (B,N,4,D_w)
        x_pdw = self.embdim_pool2(x_0_)  # (B, N, 4, Dw)

        # TODO: 3. Matrix multiplication; get X2 ~ (B,N,Dh,Dw)
        x_2 = torch.matmul(x_pdh, x_pdw)  # (B, N, 32, Dw)
        ...
        x_out = self.proj2(self.norm(x_1 + x_3))
